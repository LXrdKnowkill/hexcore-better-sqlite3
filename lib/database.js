...
function Database(filenameGiven, options) {
	...
	if (nativeBinding == null) {
		// HexCore: Use the addon pre-loaded and injected by index.js
		// This avoids any runtime dependency on node-gyp-build or bindings.
		addon = DEFAULT_ADDON || (DEFAULT_ADDON = Database._nativeAddon);
		if (!addon) {
			throw new Error(
				'hexcore-better-sqlite3: native addon not injected. ' +
				'Make sure you are requiring the package via the root index.js.'
			);
		}
